...
性格は普段の6場面から回答する。デスクトップは2列、モバイルは1列で表示し、各問に「わからない」と「その他」を用意する。「その他」を選んだ問だけ補足を入力する。保存・AI送信時は質問と回答を順序固定の文章にまとめ、5問を従来の `pet.personality`、遊び方を `pet.playStyle` に格納して文字列契約を維持する。

- 未回答で登録すると回答箇所のエラーが表示され、AI処理と保存が始まらない。
- 「その他」の補足が空白のみなら登録できず、補足欄へ戻れる。
- 「その他」から通常回答へ戻すと補足欄が消え、以前の補足を送信しない。
- キーボードで選択・補足・登録ができ、各質問とエラーの関連が読み上げに伝わる。
- 6問の通常回答および「わからない」で正常登録でき、保存した文字列に質問文と回答が含まれる。
- 390×844と1440×1000で列数、ラベルの折返し、操作領域、下部CTAを確認する。
