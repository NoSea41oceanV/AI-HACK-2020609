...
> これは `6609e68` までのモック統合時点の対応表・検証記録です。以下の「仕様不明」「BB回答待ち」は当時の状態を残した履歴です。2026-09-22に健康・社会化・7軸、当日対象、部屋設定、監査、将来カメラIFの方針が確定しました。現在の要件・先行データ契約・実装状態は [確定設計書](../ペットホテル自律AIエージェント_設計書_v4.md) と [受入条件・証拠対応表](pawpals-acceptance.md) を参照してください。手動部屋編集は今回対象外、5分類は引き続き未確定です。下記の成功記録を今回追加要件の検証済み証拠には流用しません。
