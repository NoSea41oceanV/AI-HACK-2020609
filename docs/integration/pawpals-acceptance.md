...
仕様出典はBB最新ユーザー決定の担当割当連絡。backend A/B/C、UI、App、7軸matchingを最終 `b1cb969` へ統合し、2026-09-22にローカル統合検証とFirebase Hosting / Rules / Indexes配備が完了した。
...
| I04 | matching `9b727cb` / 根拠UI・最終統合 `b1cb969` | 全7軸を最大18点として100点へ統合。適用/未適用根拠UI。19ファイル127件、typecheck/build/Rules/E2E成功 | 公開側の認証後E2Eは別 |
| D05 | Firebase配備報告 | Hosting / Rules / Indexes deploy exit 0。公開HTTP 200、asset一致、ログインUI/ready/console 0。Worker health正常 | 本番資格情報がないため公開認証後E2E未実施 |
...
|---|---|---|---|---|---|
| R01 | モック構造/配色/操作感を活用。実データまたは明示空状態を表示し、未対応/IFを区別 | UI・[09] | 検証済み | B02/M01/I03/I04 | desktop/mobile、実データ経路、console 0を確認。cameraはunsupported表示 |
...
| R07 | 全n(n-1)/2ペアを採点。両犬に7軸があれば既存信号と全7軸を100点へ統合し、欠損時は既存計算。hard制約を維持 | matching・UI・[09] | 検証済み | B01/C01/D01/I04 | 最大18点の全7軸感度、対称性/決定性、0〜100、旧データ回帰、片側欠損、EXPLICIT_BLOCKを検証 |
| R07b | `aiSevenAxisApplied`、評価内訳またはfallbackを計算結果へ保存し、最適化・再読込・%表示で同一結果を使う。5分類は追加しない | matching・UI・[09] | 検証済み | R07/I04 | E2Eで96.3%、16.3/18点、軸別内訳とUI根拠を確認 |
...
| R11 | 招待発行の施設/スタッフ契約を維持。期限なし・一度登録・hash保持・アクセス分離 | backend・UI・[09] | 検証済み | B01/B02/I03 | Rules実emulatorと招待→登録E2E成功 |
| R12 | 手動観測を保存・表示・再計算へ接続。source/scenarioIdをmanual固定、legacy read互換 | backend・UI・[09] | 検証済み | B01/C02/D03/D04/I04 | 冪等性、日付/未来、施設境界、App統合テスト成功 |
...
| R14 | 実在する観測/snapshotsのみ表示。severity/ack/交流実績/AI説明を捏造しない | UI・[09] | 検証済み | I03/I04 | 保存済み結果と7軸根拠UI、fallback表示をテスト |
| R15 | AI/保存/認証失敗と無料枠到達を明示。媒体・PII・Secret境界を維持 | backend・UI・[09] | 進行中（今回回帰待ち） | B01/B02/C01 | 未受領。拡張payload、Worker拒否/失敗、保存失敗、ログ境界 |
| R16 | 今回変更の全体統合と最終レビュー | [09]・レビュー担当 | 検証済み | R01〜R15/I04 | 最終統合 `b1cb969`、`gpt-5.6-sol / high`。全自動検証とローカルE2E成功 |
| R17 | 公開配備と実環境の保存・AI・認証・当日運用 | 配備担当 | 配備済み（認証後E2Eを除く） | D05 | Hosting/Rules/Indexes、公開asset/ログイン/Worker health確認。認証後の実保存は未実施 |
