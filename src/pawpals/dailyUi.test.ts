...
describe('daily operations display guards', () => {
  it('counts only the current unresolved plan and uses the saved daily selection', () => {
    ...
    expect(html).not.toContain('操作担当：')
    expect(html).not.toContain('最新の案を確認し、承認または却下を記録します。')
    expect(html).not.toContain('判断理由・確認メモ（却下の場合のみ必須）')
    expect(html).toContain('aria-label="却下理由"')
    expect(html).toContain('placeholder="却下する場合は、理由を入力してください。"')
